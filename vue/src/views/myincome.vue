<template>
    <div>
        <div style="padding: 10px 0">
            ...
            <el-input class="ml-5" v-model="search_fromway" style="width: 200px" suffix-icon = "el-icon-search" placeholder="输入支付方式查询"></el-input>
            <el-date-picker
                v-model="time"
                type="datetimerange"
        ...
        <el-table :data="tableData" border strip header-cell-class-name="headerBg">
            ...
            <el-table-column prop="fromway" label="收入去向" width="120">
            </el-table-column>
        ...
        <el-dialog title="收入信息" :visible.sync="add_dialogFormVisible" width="15%">
            <el-form label-width="60px" size="small">
                ...
                <el-form-item label="收入去向">
                    <el-select clearable v-model="userform.fromway" palceholder="请选择" style="width: 100%">
                        <el-option v-for="item in fromways" :key="item" :label="item" :value="item"></el-option>
                    </el-select>
                </el-form-item>
...
export default {
    ...
    methods: {
        ...
        //     this.request.delete("/incomedetail/delbatch" , {data: ids}).then(res => {
        //         if(res.code === 200){
        //             this.$message.success("删除成功");
        //             this.load();
        //         }else
        ...
        // },
        load_way() {
            this.request.get("/balance/userwayshas", {
                params: {
                    username : this.user.data.username,
                }
            }).then(res => {
                this.fromways = res.data;
            })
        }
    }
}
